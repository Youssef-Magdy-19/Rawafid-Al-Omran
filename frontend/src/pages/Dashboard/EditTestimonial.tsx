...
import { useTestimonial, useUpdateTestimonial } from '@hooks/dashboard';
...
export function EditTestimonial() {
  ...
  const { data: testimonial, isLoading, isError, error } = useTestimonial(id || '');
  ...
  const handleSubmit = (data: Record<string, unknown>) => {
    ...
    updateTestimonial.mutate(
      { id: testimonial.id, data },
      {
        onSuccess: () => {
          addToast(t('dashboard.editTestimonial.updatedSuccess'), 'success');
  ...
  return (
    <div className="space-y-6">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.1 }}>
        <TestimonialsForm initialData={testimonial} onSubmit={handleSubmit} isSubmitting={updateTestimonial.isPending} serverErrors={serverErrors} />
      </motion.div>
    </div>
  );
}
