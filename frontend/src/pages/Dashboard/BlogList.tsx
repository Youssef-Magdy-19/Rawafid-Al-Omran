...
import { mockBlogPosts, blogCategoryLabels, blogAuthorLabels } from '@data/blogMockData';
...
type SortField = 'titleEn' | 'category' | 'author' | 'publishDate' | 'createdAt' | 'published';
...
export function BlogList() {
  ...
  const filtered = useMemo(() => {
    ...
    return result;
  }, [search, categoryFilter, authorFilter, statusFilter, featuredFilter, sortField, sortOrder]);
  ...
  const hasActiveFilters = search || categoryFilter !== 'all' || authorFilter !== 'all' || statusFilter !== 'all' || featuredFilter !== 'all';
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        <button
          onClick={() => navigate(ROUTES.DASHBOARD_BLOG_ADD)}
        ...
        <div className="p-4 border-b border-border space-y-4">
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            >
              ...
              <span className="hidden sm:inline">{t('dashboard.blog.filterCategory')}</span>
            </button>
          </div>
          ...
          {showFilters && (
            ...
            >
              ...
              >
                <option value="all">{t('dashboard.blog.allCategories')}</option>
              ...
              <select
                ...
                className="h-10 px-3 rounded-lg border border-input bg-background text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring"
              >
                <option value="all">{t('dashboard.blog.allAuthors')}</option>
              ...
              <select
                ...
                className="h-10 px-3 rounded-lg border border-input bg-background text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring"
              >
                <option value="all">{t('dashboard.blog.allStatuses')}</option>
                <option value="published">{t('dashboard.blog.published')}</option>
                <option value="draft">{t('dashboard.blog.draft')}</option>
              </select>
              ...
              <select
                ...
                className="h-10 px-3 rounded-lg border border-input bg-background text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring"
              >
                <option value="all">{t('dashboard.blog.all')}</option>
                <option value="featured">{t('dashboard.blog.featuredOnly')}</option>
                <option value="nonFeatured">{t('dashboard.blog.nonFeatured')}</option>
              </select>
              ...
              <select
                ...
                className="h-10 px-3 rounded-lg border border-input bg-background text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring"
              >
                <option value="publishDate">{t('dashboard.blog.sortDate')}</option>
                <option value="titleEn">{t('dashboard.blog.sortName')}</option>
                <option value="category">{t('dashboard.blog.category')}</option>
                <option value="published">{t('dashboard.blog.sortStatus')}</option>
              </select>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border/50">
                ...
                <th className="text-left px-4 py-3 text-xs font-medium text-muted-foreground uppercase tracking-wider">{t('dashboard.blog.author')}</th>
                ...
                <th className="text-left px-4 py-3 text-xs font-medium text-muted-foreground uppercase tracking-wider">{t('dashboard.blog.createdDate')}</th>
                ...
                paginated.map((post) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-sm text-foreground font-medium">{post.titleAr}</td>
                      <td className="px-4 py-3 text-sm text-muted-foreground">{post.titleEn}</td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-medium ${
                          ...
                          {post.published ? t('dashboard.blog.published_badge') : t('dashboard.blog.draft_badge')}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        {post.featured ? (
                      ...
                      <td className="px-4 py-3 text-sm text-muted-foreground">{post.publishDate || '—'}</td>
                      <td className="px-4 py-3 text-sm text-muted-foreground">{post.createdAt}</td>
                      <td className="px-4 py-3" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center justify-end gap-1">
                          <button
                            onClick={() => navigate(`/dashboard/blog/${post.id}`)}
                            className="rounded-lg p-1.5 text-muted-foreground hover:bg-muted hover:text-foreground transition-colors"
                            title={t('dashboard.blog.view')}
                          >
