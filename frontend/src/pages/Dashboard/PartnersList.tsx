...
import {
  ...
  ChevronLeft, ChevronRight, X, ArrowUpDown, Globe, Handshake,
  Send, Ban,
} from 'lucide-react';
import { ROUTES } from '@constants/route.constants';
import { usePartnersList, useDeletePartner, useUpdatePartner } from '@hooks/dashboard';
...
export function PartnersList() {
  ...
  const { data, isLoading, isError, refetch } = usePartnersList(queryParams);
  ...
  const allSelected = partners.length > 0 && partners.every((p) => selectedIds.has(p._id));
  ...
  const toggleSelectAll = () => {
    ...
    else setSelectedIds(new Set(partners.map((p) => p._id)));
  };
  ...
  const handleBulkDelete = () => {
    selectedIds.forEach((id) => deletePartner.mutate(id));
    addToast(t('dashboard.partners.bulkDeleted'), 'success');
    setSelectedIds(new Set());
  };

  const handleBulkAction = (action: 'activate' | 'deactivate') => {
    selectedIds.forEach((id) => {
      updatePartner.mutate({ id, data: { isActive: action === 'activate' } });
    });
    const msg = action === 'activate' ? 'bulkActivated' : 'bulkDeactivated';
    addToast(t(`dashboard.partners.${msg}`), 'success');
    setSelectedIds(new Set());
  };
  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}
        ...
        <button onClick={() => navigate(ROUTES.DASHBOARD_PARTNERS_ADD)}
          ...
          <Plus className="h-4 w-4" />{t('dashboard.partners.addNew')}
        </button>
      </motion.div>
        ...
        <div className="p-4 border-b border-border space-y-4">
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <button onClick={() => setShowFilters(!showFilters)}
              ...
              <SlidersHorizontal className="h-4 w-4" /><span className="hidden sm:inline">{t('dashboard.partners.sortBy')}</span>
            </button>
          </div>
          ...
          {showFilters && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="flex flex-wrap gap-3 pt-2">
              <Dropdown
                ...
                placeholder={t('dashboard.partners.all')}
                options={[
                  { value: 'active', label: t('dashboard.partners.active') },
                  { value: 'inactive', label: t('dashboard.partners.inactive') },
                ]}
                className="w-44"
              />
              <Dropdown
                ...
                placeholder={t('dashboard.partners.sortName')}
                options={[
                  { value: 'name', label: t('dashboard.partners.sortName') },
                  { value: 'createdAt', label: t('dashboard.partners.sortDate') },
                ]}
                className="w-44"
              />
              <button onClick={() => setSortOrder(sortOrder === 'asc' ? 'desc' : 'asc')}
                ...
                <ArrowUpDown className="h-4 w-4" />{sortOrder === 'asc' ? t('dashboard.partners.ascending') : t('dashboard.partners.descending')}
              </button>
              {hasActiveFilters && (
            ...
            <tbody className="divide-y divide-border/50">
              {partners.length === 0 ? (
                ...
                partners.map((p) => {
                  const pid = p._id;
                  return (
                    <tr key={pid} className="hover:bg-muted/50 transition-colors cursor-pointer"
                      onClick={() => navigate(ROUTES.DASHBOARD_PARTNERS_DETAILS.replace(':id', pid))}>
                      <td className="px-4 py-3" onClick={(e) => e.stopPropagation()}>
                        <input type="checkbox" checked={selectedIds.has(pid)} onChange={() => toggleSelect(pid)}
                          className="h-4 w-4 rounded border-input text-primary focus:ring-primary" />
                      </td>
                      <td className="px-4 py-3">
                        <div className="h-9 w-9 rounded-lg overflow-hidden border border-border bg-muted">
                          {p.logo && <img src={p.logo} alt="" className="h-full w-full object-cover"
                      ...
                      <td className="px-4 py-3">
                        ...
                        {!p.website && <span className="text-sm text-muted-foreground">—</span>}
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center justify-end gap-1">
                          <button onClick={() => navigate(ROUTES.DASHBOARD_PARTNERS_DETAILS.replace(':id', pid))}
                            className="rounded-lg p-1.5 text-muted-foreground hover:bg-muted hover:text-foreground transition-colors" title={t('dashboard.partners.view')}>
                            <Eye className="h-4 w-4" />
                          </button>
                          <button onClick={() => navigate(ROUTES.DASHBOARD_PARTNERS_EDIT.replace(':id', pid))}
                            className="rounded-lg p-1.5 text-muted-foreground hover:bg-muted hover:text-primary transition-colors" title={t('dashboard.partners.edit')}>
                            <Pencil className="h-4 w-4" />
                          </button>
                          <button onClick={(e) => handleDelete(e, pid)} disabled={deletePartner.isPending}
                            className="rounded-lg p-1.5 text-muted-foreground hover:bg-muted hover:text-destructive transition-colors disabled:opacity-50" title={t('dashboard.partners.delete')}>
