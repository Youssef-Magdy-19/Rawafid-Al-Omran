...
import { useServicesList, useDeleteService } from '@hooks/dashboard';
...
const iconMap: Record<string, React.ComponentType<{ className?: string }>> = {
  HardHat, Ruler, Truck, Wrench, Building2, PencilRuler, Drill, PaintBucket, Shovel,
};
...
export function ServicesList() {
  ...
  const queryParams = useMemo(() => {
    ...
    return params;
  }, [search, activeFilter, featuredFilter, sortField, sortOrder, currentPage]);

  const { data, isLoading, isError, refetch } = useServicesList(queryParams);
  ...
  const handleDelete = (e: React.MouseEvent, id: string) => {
    ...
    deleteService.mutate(id, {
      onSuccess: () => {
        addToast(t('dashboard.services.deletedSuccess'), 'success');
  ...
  const handleBulkDelete = () => {
    ...
    addToast(t('dashboard.services.bulkDeleted'), 'success');
  ...
  const hasActiveFilters = activeFilter !== 'all' || featuredFilter !== 'all' || search;
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        <button onClick={() => navigate(ROUTES.DASHBOARD_SERVICES_ADD)}
        ...
        >
          <Plus className="h-4 w-4" />{t('dashboard.services.addNew')}
        </button>
      </motion.div>
      ...
      >
        <div className="p-4 border-b border-border space-y-4">
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            >
              <SlidersHorizontal className="h-4 w-4" /><span className="hidden sm:inline">{t('dashboard.services.filter')}</span>
            </button>
          </div>
          ...
          {showFilters && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="flex flex-wrap gap-3 pt-2">
              <Dropdown
                ...
                placeholder={t('dashboard.services.allStatus')}
                options={[
                  { value: 'active', label: t('dashboard.services.active') },
                  { value: 'inactive', label: t('dashboard.services.inactive') },
                ]}
                className="w-44"
              />
              <Dropdown
                ...
                placeholder={t('dashboard.services.all')}
                options={[
                  { value: 'featured', label: t('dashboard.services.featuredOnly') },
                  { value: 'nonFeatured', label: t('dashboard.services.nonFeatured') },
                ]}
                className="w-44"
              />
              <Dropdown
                ...
                placeholder={t('dashboard.services.sortOrder')}
                options={[
                  { value: 'order', label: t('dashboard.services.sortOrder') },
                  { value: 'title', label: t('dashboard.services.sortName') },
                  { value: 'createdAt', label: t('dashboard.services.sortDate') },
                ]}
                className="w-44"
              />
              ...
              >
                <ArrowUpDown className="h-4 w-4" />{sortOrder === 'asc' ? t('dashboard.services.ascending') : t('dashboard.services.descending')}
              </button>
              {hasActiveFilters && (
        ...
        {selectedIds.size > 0 && (
          <div className="px-4 py-3 bg-primary/5 border-b border-primary/10 flex items-center gap-3">
            <span className="text-sm font-medium text-foreground">{selectedIds.size} {t('dashboard.services.selected')}</span>
            <div className="h-4 w-px bg-border" />
            <button onClick={handleBulkDelete} className="text-sm text-destructive hover:underline flex items-center gap-1">
              <Trash2 className="h-3.5 w-3.5" />{t('dashboard.services.bulkDelete')}
            </button>
          </div>
        )}
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border/50">
                ...
                <th className="text-left px-4 py-3 text-xs font-medium text-muted-foreground uppercase tracking-wider">{t('dashboard.services.serviceName')}</th>
            ...
            <tbody className="divide-y divide-border/50">
              {services.length === 0 ? (
                ...
                services.map((service) => {
                  ...
                  return (
                    <tr key={service.id} className="hover:bg-muted/50 transition-colors cursor-pointer"
                      onClick={() => navigate(`/dashboard/services/${service.slug}`)}
                    >
                      <td className="px-4 py-3" onClick={(e) => e.stopPropagation()}>
                        <input type="checkbox" checked={selectedIds.has(service.id)} onChange={() => toggleSelect(service.id)}
                          className="h-4 w-4 rounded border-input text-primary focus:ring-primary" />
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <span className="text-sm font-medium text-foreground">{isAr ? service.titleAr : service.title}</span>
                      </td>
                      <td className="px-4 py-3">
                        {service.isFeatured ? (
                      ...
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-medium ${
                          service.isActive ? 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/20' : 'bg-muted text-muted-foreground border-border'
                        }`}>
                          {service.isActive ? (
                            <><Power className="h-3 w-3 mr-1" />{t('dashboard.services.active')}</>
                          ) : (
                            <><PowerOff className="h-3 w-3 mr-1" />{t('dashboard.services.inactive')}</>
                          )}
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-3" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center justify-end gap-1">
                          <button onClick={() => navigate(`/dashboard/services/${service.slug}`)}
                            className="rounded-lg p-1.5 text-muted-foreground hover:bg-muted hover:text-foreground transition-colors" title={t('dashboard.services.view')}>
                            <Eye className="h-4 w-4" />
                          </button>
                          <button onClick={() => navigate(`/dashboard/services/${service.slug}/edit`)}
                            className="rounded-lg p-1.5 text-muted-foreground hover:bg-muted hover:text-primary transition-colors" title={t('dashboard.services.edit')}>
                            <Pencil className="h-4 w-4" />
                          </button>
                          <button onClick={(e) => handleDelete(e, service.id)} disabled={deleteService.isPending}
                            className="rounded-lg p-1.5 text-muted-foreground hover:bg-muted hover:text-destructive transition-colors disabled:opacity-50" title={t('dashboard.services.delete')}>
                            {deleteService.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />}
                          </button>
                        </div>
                      </td>
                    </tr>
